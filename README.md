...
Leaving a player-vs-player fight has two house-rule exits. **Retreat** (or simply losing the battle): the loser pays **5 gold to the winner — and may drop into debt (gold can go negative)** — takes a negative morale token, loses troops per the mode above, hands the opponent a win (experience, Necromancy and a step toward the "defeat every enemy hero" victory), then falls back to a friendly Town or Settlement. **Surrender** (only at the start of combat, and only with the full **10 gold** in hand): pay 10 gold to the opponent, **keep your whole army in either mode**, take no morale hit, return home — and it is **not** counted as a victory for the opponent. **Shackles of War** locks an enemy out of Surrender, but they can still Retreat.
...
- **Astrologers rounds**: every even round draws one of the 28 Astrologers Proclaim cards (19 core + 9 expansion) and the engine enforces it — movement and hand-limit modifiers, next-resource-round income shifts, morale handouts, plagues, first-spell bonuses, die rerolls, neutral-draw swaps, Statistic→Empowered swaps (Dancing Imp / Hero), hand/discard card removal (Plane Between Planes), and the Rampart war-machine pair (Ammo Cart buffs every Ballista / First Aid Tent / ranged reroll; McGiver hands out a free War Machine next round). The active card sits in the HUD; the deck's past draws are browsable.
