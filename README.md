...
- **Dragon Hunt** — win either way: **defeat the Dragon Utopia** — no need to hold it afterwards — or **beat every enemy hero** in combat at least once (only 2 of the 3 in a 4-player game). A Dragon Utopia is seeded on a Center tile. Its guard is a house-rule knob set beside the Win condition: **four dragons** (azure + rust + crystal + faerie, the default) or the **rulebook three** (azure + crystal + black).
