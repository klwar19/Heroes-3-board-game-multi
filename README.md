...
- **Conquer** — win through distinct PvP victories (2 players: 1 rival; 3–4: 2 rivals; 5–6: 3 rivals; the target is fixed from the starting table, departures award no credit, and beating every surviving rival also completes it), or eliminate every enemy faction. Only this mode counts PvP wins toward an immediate victory.
- **Grail Hunt** — win either way: **capture the Grail** (defeat its level-VII guard, spend 1 more movement point to *dig* it, then carry it home to your own town), or **eliminate every enemy faction**. A Grail is seeded on a Center tile; the Dragon Utopia is **not** an objective here.
- **Dragon Hunt** — win either way: **defeat the Dragon Utopia** — no need to hold it afterwards — or **eliminate every enemy faction**. A Dragon Utopia is seeded on a Center tile. Its guard choice is **Default** (keep a designer guard if present; otherwise draw 2 azure and 2 gold Neutral cards), **2 Azure + 2 Gold** (force those four draws), **All dragons** (Azure, Rust, Crystal and Faerie), or **Difficulty table** (the full level-VII row: Easy 1 / Normal 2 / Hard 3 / Impossible 4). Dragon Conqueror uses the same guard choices.
- **Dragon Conqueror** — defeating the Dragon Utopia **captures** it instead of winning outright. The holder garrisons it and rivals must **besiege** it (Walls, Gate, Arrow Tower) to take it; holding it through the end of the round following capture wins.
