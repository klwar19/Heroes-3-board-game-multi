...
- Units have attack, defense, health points, and initiative. Higher initiative activates first. Same-speed ties (`getActivationStep` + `advanceActiveUnit`): **the attacker is always prioritised** — every attacker unit at a given initiative activates before any defender (or Neutral) unit at that same initiative, so a defender unit lifted into a tie (e.g. a Cyclopes boosted by Expert Archery / an Ammo Cart) can never slip ahead of the attacker's same-speed units. This only breaks ties: a real speed gap always wins, so a defender that strictly OUTSPEEDS the attacker still leads. When several of ONE side's units tie for the slot, that side's controller is prompted to choose which goes first (`combat-activation-order` choice); a real player picks among their own tied units, and since the Neutral army cannot answer a prompt the player running the fight (the attacker — the player is always the attacker against guards) breaks the Neutral tie on its behalf, exactly as the attacker already breaks a Neutral unit's target ties. Units still act one at a time (a faster resolution can still remove a tied enemy before it acts). The initiative rail (`getActivationOrder`) steps this same logic, so the displayed order always matches how the round actually activates.
