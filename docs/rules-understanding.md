...
- Defense tokens (rulebook p.42): defending replaces attacking and ends the activation. The token is discarded at the start of the unit's **next activation** — it survives the end of the combat round. A unit may **not** Defend on two consecutive activations — after Defending it must do something else (attack, move, hold, …) before it may Defend again (`defendedLastActivation`, `consecutive-defend.test.ts`).
