...
- Every implemented Artifact, Spell and Ability is now dealt into the shared draw decks — including the previously library-only Chain Lightning, Blind, Greater Gnoll's Flail, Mystic Orb of Mana, Charm of Mana, Shackles of War, Shield of the Dwarven Lords and Orb of Vulnerability. The invariant is enforced by `deck-coverage.test.ts` (which also fails if a not-implemented card is ever placed in a deck).
...
- Still inert / not in the decks (no engine effect yet): Teleport, Mirth, Sorrow, Slayer, Forgetfulness, Inferno, Visions; Pathfinding, Learning
- Now implemented AND in the shared draw decks: Necromancy, Magic Mirror, Intelligence, Artillery (basic shot + expert Ballista 3× volley), Tactics, Diplomacy, Dimension Door, the Schools of Magic permanents (BINH ability deck), Blind, Chain Lightning, and the artifacts Shield of the Dwarven Lords, Orb of Vulnerability, Charm of Mana, Greater Gnoll's Flail, Mystic Orb of Mana, Shackles of War — see content-tracker.md. Every implemented Artifact/Spell/Ability is now deck-reachable (enforced by `deck-coverage.test.ts`).
