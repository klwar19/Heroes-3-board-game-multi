...
- ✅ 66 neutral units in four tier decks (bronze 29 / silver 20 / gold 14 / azure 3), derived into the tier decks by `neutralUnitIdsByTier` (`core.ts`). Covers the core + Inferno/Rampart/Stronghold expansion neutrals (incl. Faerie Dragons azure) plus the Tower/Fortress/Conflux creatures that only appear as neutral guards: **bronze** Gnolls, Gremlins, Gargoyles, Lizardmen, Iron Golems, Sprites, Dragon Flies, Air/Ice/Storm Elementals; **silver** Basilisks, Gorgons, Genies, Magi, Energy/Fire/Magma Elementals. Stats and ability text transcribed per-unit from the wiki (`https://en.homm3bg.wiki/towns/neutral/` + each unit page). Each new unit's bare name is mapped to its original-game voice in `unit-sounds.ts`. Sprites use the implemented **no-retaliation** ability and Magi the **no-adjacent-shot penalty** ability; every other new ability (gargoyle paralysis-immunity, golem/elemental spell immunity, basilisk/gorgon on-hit rolls, Dragon Fly retaliation penalty, Genie anti-Efreet bonus, Magi card discard) is display-only `abilityText` — no engine claim. Still missing as neutrals: the gold/azure Tower/Fortress/Conflux creatures (Naga, Titans, Mighty Gorgons, Wyverns, Hydras, Phoenix…) and the Cove/sea units (Sea Dogs, Seamen, Oceanids, Nix, Ayssids, Fangarm, Leprechauns, Satyrs), which have no converted voice set yet.
