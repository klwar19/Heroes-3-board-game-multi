...
describe("eversmoking-ring-of-sulfur-major house rule", () => {
  it("BINH defaults the Ring's effective tier and deck to Major (Torso pattern: static tier = rule-ON reading)", () => {
    const state = makeGame();

    // Like Torso of Legion, the STATIC tier is the default (rule-ON) reading,
    // so the reads not routed through effectiveArtifactTier (AI valuation,
    // tier chip) agree with the default deck placement.
    expect(cardLibrary[RING]?.artifactTier).toBe("major");
