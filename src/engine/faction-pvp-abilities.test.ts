 ...
 *    each of three one-off combat counters (discard a random LB card, deal 3 to
 *    the LB campus hero unit, draw a card).
 *  - Monster Girl Quest: the OPPONENT of an MGQ seat draws 1 card at battle start.
 */
...
describe("Little Busters PvP counters", () => {
  ...
  it("the damage counter deals exactly 3 to the LB hero unit for 1 gold", () => {
    ...
    expect(next.players.p1.resources.gold).toBe(goldBefore - 1);
    expect(next.combat!.units.unit_p2_campus_hero.damage).toBe(before + 3);
    expect(counterAction(next, "p1", "damage")).toBeUndefined();
  });

  it("the draw counter draws 1 card off the fighter's own deck for 1 gold", () => {
...
describe("Monster Girl Quest opponent battle-start draw", () => {
  ...
  it("the opponent of an MGQ seat draws exactly 1 card as the battle begins", () => {
    ...
    expect(state.players.p2.hand.length, "the opponent drew one card at battle start").toBe(handBefore + 1);
    expect(state.combat?.mgqOpponentBattleStartDrawDone).toContain("p2");
  });
