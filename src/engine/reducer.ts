 ...
 * reducedSpellDamage.
 */
function reducedCardDamage(unit: CombatUnitState, card: CardDefinition | undefined, amount: number): number {
  ...
  if (stackItem.action.type === "CAST_SPELL") {
    ...
    if (card?.effect.type === "AREA_DAMAGE_ADJACENT" && state.combat && stackItem.action.target.type === "unit") {
      ...
      if (target) {
        ...
        const dealt = reducedCardDamage(target, card, amount);
        target.damage += dealt;
        noteUnitDamagedForTokens(state, target, dealt);
        appendEvent(state, {
          type: "DAMAGE_ASSIGNED",
...
function dealChainLightningDamage(
  ...
  const dealt = reducedCardDamage(unit, card, amount);
  if (dealt <= 0) {
    return;
  }
  unit.damage += dealt;
  ...
  if (effect.type === "AREA_DAMAGE_ALL_ADJACENT" && target && state.combat) {
    ...
    if (center) {
      ...
      for (const unit of inBlast) {
        const dealt = reducedCardDamage(unit, card, effect.amount);
        unit.damage += dealt;
        noteUnitDamagedForTokens(state, unit, dealt);
        appendEvent(state, {
          type: "DAMAGE_ASSIGNED",
