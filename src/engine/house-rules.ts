...
export const HOUSE_RULES: HouseRuleDef[] = [
  ...
  },
  {
    ...
    description:
      "Polish house rule: the control / enchantment spells — Anti-Magic, Blind, Frenzy, Sorrow, Disrupting Ray — MAY be cast on a tierless Creature-Bank unit (a bank GUARD such as the Nagas in a Naga Bank, AND a won \"gain a unit\" bank REWARD card such as Dragon Flies). The bank unit is targeted at its UNDERLYING grade (capped at gold), so Power still matters — a gold Naga needs a gold-reaching cast, a bronze Dragon Flies is reachable at 2 Power. Off (default): every tier-gated spell treats a bank unit as gradeless and cannot reach it. Berserk / Teleport / Clone and tier-gated damage spells stay blocked on a bank unit either way, and this never extends to commanders or heroes.",
    category: "polish",
    default: false,
    legacyDefault: false
  },
  ...
  },
  {
    ...
    description:
      "Community house rule (default OFF in both modes): the \"Heroes 3 Board Game Community Balance Change\" reprints — the community balance spreadsheet's Abilities, Spells, Artifacts, Units and War Machines. A covered card plays its NEW printed text and shows its community card face; every card that is NOT covered is byte-identical to a normal game, face included. SCOPE: the pack is COMPLETE — all twelve of the sheet's abilities (each with its Empowered printing except Mysticism, which has no Expert side), all twenty-six spells, all thirty-four artifacts, the four changed unit sides and the three re-priced war machines. Estates gives 2 / 4 gold instead of 3 / 6. Leadership's Expert side draws 2 cards and no longer also grants a Morale token. Scouting is a flat Search (4) basic / Search (5) Expert, and the Expert side now REMOVES the card from the game. Artillery's basic side drops the lowest-initiative restriction — 1 damage to any enemy unit, played on your own Combat activation rather than as an instant reaction — and its Expert Ballista volley now lets you pick the target. Ballistics' basic side pays 1 building material to hit 2 adjacent targets (units, Walls or the Gate) at any point in Combat, and its Expert side fires your Catapult twice on the same targets for free. First Aid draws a card on BOTH sides, and its Tent triple-volley costs a crown again. Wisdom stops being a Town card: it is an instant played into your own Spell cast for +1 Power (Expert +2) that also raises this Combat round's Spell limit by 1, so that cast does not count against it. Luck lasts THIS TURN instead of the game round, and its reroll budget for a die kind is one per die rolled instead of one per die kind for the whole round. Mysticism's basic recall also takes back one card played alongside the Spell. Tactics loses its start-of-Combat window: the basic side is the mid-combat swap and costs no crown, and the Expert side (a crown) lets you swap when ANY unit — including an enemy's — is about to activate. Intelligence stops granting a timing freedom: it is now a cast enabler — play any Spell sitting in your OWN discard pile, at your normal Power, through the ordinary spell pipeline; the basic side counts toward the one-Spell-per-Combat-round limit and the Expert side (a crown) does not. LIMIT: like every other cast in this engine it is not offered while a reaction window is already open. Necromancy becomes \"Recruit OR Reinforce a unit you have the corresponding DWELLING for, at half the gold cost rounded down\" — a new Few of any roster unit whose Dwelling is built is now buyable in the after-combat window, and the reinforce arm is dwelling-gated too (so a recruited Neutral or a won Creature-Bank card is no longer eligible). It always resolves inside that window and never banks a discount, so the \"Immediate reinforcement prompts\" toggle is ignored while this pack is on. SPELLS: all 26 of the sheet's spells are reprinted. Ladder bumps — Precision +2/+3/+4 attack, Shield and Stone Skin +2/+3/+4 defense, Bloodlust +2/+3/+4 attack, Curse -2/-3/-4 defense, Weakness -2/-3/-4 attack, Haste +3/+6/+9 initiative. Breakpoint moves 0/2/4 → 0/1/2 for Counterstrike, Anti-Magic, Town Portal, Fire Wall, Frenzy and Mirth; 0/1/2 → 0/2/4 for Bless and Forgetfulness. Haste also becomes an INSTANT and prints \"+1 space\"; Slow prints \"moves 1/2/3 fewer spaces\" (both apply whether or not the classic \"Haste/Slow shift Combat movement\" rule is on). Fortune stops rerolling and SETS a die to \"+1\" (1/2/3 uses) — its map Treasure/Resource half still rerolls. View Air's Power-0 rung discovers an adjacent Map tile instead of paying 3 gold. Chain Lightning's Power-0 rung is 1/1/0 (the third target takes nothing). Visions scrys 2/4/6 and puts every card back on the TOP or the BOTTOM of the deck — nothing is discarded. Fire Wall also burns a unit that STARTS its activation on it. Misfortune is reworked into an ongoing that SETS the die on the next 1/2/3 ENEMY attack rolls. Inferno deals a flat 1 damage on the chosen space before the dice and rolls 3 (not 4) at the top rung. Slayer is reworked into +2/+4/+6 attack when attacking a HIGHER-TIER unit. Bless lasts the whole Combat and drops its ground/flying restriction. Forgetfulness blocks only NON-ADJACENT (ranged) attacks, for 1/2/3 activations, at any tier. Dispel is reworked into \"discard 1/2/3 active ongoing effects or Paralysis tokens\" of your choice, from any owner. Prayer becomes a lasting +1/+2/+3 to attack AND defense AND initiative. Cure's reprint only writes \"friendly\" into text the engine already enforced — no behaviour change. ARTIFACTS: all 34 of the sheet's artifacts are reprinted. Boots of Polarity ignores the enemy Spell with no dice roll. Celestial Necklace of Bliss / Sword of Judgement / Lion's Shield / Sandals of the Saint gain a flat +1 base on their discard-X sides (and the Necklace's scaling half turns into Defense laid on YOUR OWN unit for the combat round). Crown of Dragontooth recovers UP TO 2 Spells. Endless Sack of Gold becomes a 4-gold income PERMANENT (remove for 5); the Inexhaustible Cart of Ore pays 2 materials a round. Endless Bag of Gold, the Everpouring Vial of Mercury and the Inexhaustible Cart of Lumber become ONGOING cards that pay 3 gold / 1 valuables / 2 materials at the END OF EACH of your turns (they sit in the Permanents & Ongoing tray and take no permanent slot). Endless Purse of Gold and the Everflowing Crystal Cloak re-price their two sides around a discard. The Eversmoking Ring of Sulfur moves from MINOR to MAJOR (deck AND tier read) and its remove side pays 1 valuables. Golden Bow trades the ranged combat-penalty waiver for an Attack-die reroll on each ranged attack. Ambassador's Sash gains a positive Morale token instead of a die reroll, and Cards of Prophecy becomes a pre-Search \"do Search (4) instead\" — so NEITHER is offered in a die-reroll window any more (the Diplomat's Ring still is). The five Legion pieces re-price their discounts (1 valuables or 4 gold for Arms/Head, 3 gold for Legs/Loins/Torso) and their second sides become map plays that remove the card to Reinforce a bronze/silver/gold unit at a reduced price. Ogre's Club of Havoc discards 2 for +3 attack and returns to hand; Targ of the Rampaging Ogre pays +3 defense. Pendant of Second Sight removes a Paralysis token AND wards the unit in one play, or Searches (2) your own deck. Surcoat of Counterpoise becomes +1 / (discard 1) +2 defense that also reduces Spell damage. Tunic of the Cyclops King pays +3 Power; Scales of the Greater Basilisk +2. Breastplate of Petrified Wood's Power side also frees the next Spell from the round limit. Centaur's Axe is used AFTER the Attack die roll. Dragon Wing Tabard becomes +1 Power & draw, or return a Spell from your discard. Hourglass of the Evil Hour is +1 defense, or a combat-round-long \"every +1 Attack die result counts as 0\" for both armies. Speculum's remove side Searches (1) the Artifact, Spell or your own deck. Spirit of Oppression becomes a map play giving EVERY player negative Morale. UNITS: four Castle sides change (Castle only — no other faction is on the sheet). Griffins gain 1 Defense on BOTH the Few and the Pack side (printed 0), so with the older \"Griffins: +1 Attack (Few)\" toggle also on the Few fights at 3 Attack AND 1 Defense; the Pack of Marksmen has 3 Health (printed 2); and the Pack of Halberdiers' Parry loses its cost — \"when the unit is targeted by any attack, you can ignore the Attack die's roll result\" with NO discard, so it works with an empty hand. These are runtime side overrides (printed unit data is never edited) and they hold whatever the older per-unit toggles say. WAR MACHINES: the Ammo Cart costs 3 gold at the Blacksmith / War Machine Factory and 5 at the Trading Post (printed 5 / 8), the Ballista 4 and 6 (printed 7 / 10), and the First Aid Tent 5 and 7 (printed 3 / 6 — a price RISE). The Catapult and the Cannon are not on the sheet and keep their printed prices; so do the 5-gold specialty war-machine grants. The machine-readable scope is COMMUNITY_BALANCE_CARD_IDS / COMMUNITY_BALANCE_NOT_IMPLEMENTED (cards) and COMMUNITY_BALANCE_UNIT_FACES (unit sides) in src/data/cards/community-balance-art.ts — an entry is listed there ONLY once its new behaviour is genuinely engine-wired, so a face can never advertise a rule the engine does not run. PRECEDENCE: if BOTH this rule and the Polish \"Balanced cards (Balance Pack)\" rule are on, the COMMUNITY reprint and face WIN for any card both packs cover (the community swap is applied AFTER the polish swap at every seam).",
    category: "community",
    default: false,
    legacyDefault: false
  },
