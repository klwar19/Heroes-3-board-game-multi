 ...
 * Home-Way's full Town-Portal price table (here a flat one-gold controlled-holding fare),
 ...
 * guarding it), the Adventure Cave's dungeon crawl (here an escalating Ⅱ→Ⅲ→Ⅳ
...
export const WOG_FIELD_OVERRIDE_DEFINITIONS: Record<string, FieldOverrideDefinition> = {
   ...
   * pay 10 gold for a commander stat point (only with the Commanders module on
   * and a commander), pay 3 gold for +1 hero experience, pay 4 gold to drill one
  ...
  emerald_tower: {
    ...
    summary:
      "Guarded (Ⅲ). After the win: pay 10 gold for +1 commander stat point (Commanders module only), 3 gold for +1 hero experience, or 4 gold to teach one chosen army unit card +2 unit XP (Unit Experience rule only).",
    image: art("emerald_tower")
  },

  /**
   * Mirror of the Home-Way — WoG's Town-Portal-for-a-price object (board-
   * adapted). Visit: pay 1 gold to teleport the hero to one of your controlled
   * Towns, Settlements, Mines or Random Towns, then gain +1 morale. The visitor
   * picks when several are reachable. The teleport reuses the Town-Portal
   * `TELEPORT_HERO` machinery. With no reachable destination no pay arm is offered.
   */
  mirror_home_way: {
    id: "mirror_home_way",
    ...
    implementationStatus: "implemented",
    summary:
      "Pay 1 gold to teleport your Hero to one of your controlled Towns, Settlements, Mines or Random Towns, then gain +1 morale.",
  ...
  /**
   * Adventure Cave — WoG's escalating dungeon (board-adapted). Guarded Ⅱ on
   ...
   * the hex one difficulty higher (Ⅱ→Ⅲ→Ⅳ). After the 3rd win it is cleared for
  ...
  adventure_cave: {
    ...
    implementationStatus: "implemented",
    summary:
      "Escalating fight (guarded Ⅱ→Ⅲ→Ⅳ). Win 1: +3 gold. Win 2: place a chosen Stack Token on an eligible army unit card, or roll a Treasure die if none qualifies. Win 3: Search (1) Artifact plus the commander-artifact rider when enabled. Cleared after the 3rd win.",
    image: art("adventure_cave")
  },

  /**
   * Altar of the Gods — WoG's God's-Altar sacrifice (board-adapted). Pay 3
   * valuables, then choose a blessing: +1 morale, +2 hero experience, or — with
   * Unit Experience enabled — +3 unit XP to one chosen army unit card. Revisitable
   * (1 MP), no guard. Menu built in `buildWogFieldVisitStep`.
   ...
   * point AND +1 morale (Commanders module + a commander) or +3 hero experience.
   */
  altar_of_gods: {
    id: "altar_of_gods",
    ...
    implementationStatus: "implemented",
    summary:
      "Pay 3 valuables, then choose +1 morale, +2 hero experience, or +3 unit XP to one chosen army unit card (Unit Experience only). Or make a greater sacrifice (needs 2+ army unit cards): remove one chosen unit card from the game for +3 hero experience, or +1 commander stat point AND +1 morale when Commanders are enabled.",
