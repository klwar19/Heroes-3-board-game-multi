...
export const extraAbilityCards: CardLibrary = {
  ...
  "ability.intelligence": {
    ...
    tags: [
      ...
      "magic",
      "spell-timing",
      "Instant (Combat): During Combat, before any unit activates, play a Spell card. You can still only play a Spell card during a Combat round. Expert: this Spell does not count toward your Spell limit per Combat round.",
      "USER RULING 2026-09-23 (matches the printed card): Intelligence is a ONE-SHOT round-start cast, not a combat-long timing freedom. It is playable only before any unit activates in the round; resolving it opens the Intelligence window, in which the holder casts one Spell now or skips (the card is spent either way). That cast carries the Instant symbol, so Helm of Chaos may cancel it.",
  ...
  // Ⅵ/Ⅶ centre band is reachable when the Polish strength rule forces a fight.
  // In a normal game the classic level > difficulty Quick Combat resolves first
  // across the full ladder (so level 7 skips Ⅵ without spending Diplomacy); an
  // UNDER-levelled hero is still refused.
  "ability.diplomacy": {
