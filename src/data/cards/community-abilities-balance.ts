...
export const communityBalanceAbilityCards: CardLibrary = {
  ...
  "ability.mysticism": {
    ...
    tags: [
      ...
      "Community pack: the BASIC recall now also returns ONE of the cards played alongside the Spell (the Power / cost cards spent into that cast), on the MAP as well as in combat — the map recall offer and its resolution used to read the RAW printed library and could not see this side at all. The Expert side is unchanged — it still returns every card played alongside. WHICH card comes back is the PLAYER'S CHOICE on BOTH surfaces (USER RULING 2026-08-23): on the MAP the offer lists one option per candidate (`offerMapSpellKnowledgeRecall`), and in COMBAT the cast's resolution opens a `recall-alongside-pick` OPTION_CHOICE — one option per recoverable candidate — owned by the caster; the chosen card returns, the rest stay in the discard pile. Exactly one candidate returns automatically (no window), none leaves the recall spell-only."
    ],
    effect: {
      type: "RECALL_SPELL",
  ...
  "ability.necromancy": {
    ...
    tags: [
      ...
      "map",
      "necropolis-only",
      "Basic: Play after winning Combat other than Quick Combat. You can Recruit or Reinforce an Undead bronze or silver unit that you have the corresponding Dwelling for, paying half the gold cost (rounded down). Expert: any Undead unit.",
