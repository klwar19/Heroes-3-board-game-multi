...
import type { BankSize, StackTokenStat } from "@/engine/state";
...
function seq(...parts: LocationInteraction[]): LocationInteraction {
  const interactions = parts.filter((part) => part.type !== "NONE");
  ...
  return { type: "SEQUENCE", interactions };
}

function gainResources(parts: {
  gold?: number;
  valuables?: number;
  buildingMaterials?: number;
}): LocationInteraction {
  const gold = parts.gold && parts.gold > 0 ? parts.gold : undefined;
  const valuables = parts.valuables && parts.valuables > 0 ? parts.valuables : undefined;
  const buildingMaterials =
    parts.buildingMaterials && parts.buildingMaterials > 0 ? parts.buildingMaterials : undefined;
  if (!gold && !valuables && !buildingMaterials) {
    return { type: "NONE" };
  }
  return {
    type: "GAIN_RESOURCES",
    ...(gold !== undefined ? { gold } : {}),
    ...(valuables !== undefined ? { valuables } : {}),
    ...(buildingMaterials !== undefined ? { buildingMaterials } : {})
  };
}
