...
describe("Polish Banks guardian and reward scans", () => {
  it("ships every supplied card as a distinct compressed WebP", () => {
    ...
    for (const [key, side] of entries) {
      ...
      expect(hasMediaFile(side.cardImage!), `${side.cardImage} must be published — run npm run media:publish`).toBe(true);
      const bytes = assetBytes(side.cardImage!);
    ...
    const totalBytes = paths.reduce((sum, assetPath) => sum + assetBytes(assetPath), 0);
