...
export function ChatPanel({
  ...
  // Always start COLLAPSED (a FAB) on every surface — lobby, map, combat, phone
  // and desktop alike. The dock floats over events/dialogs, so it opens only
  // when the player asks for it; unread lines still badge and toast the FAB.
  const [open, setOpen] = useState(false);
  // The pre-game prompt can flip the mode AFTER this panel mounted. The panel
  // already starts collapsed, so this only matters if the player OPENED chat and
  // then switches to Phone: collapse once on the computer→phone edge. A desktop
  // session (mode never flips) is untouched.
  ...
  // Seeded immediately so "time ago" labels are ready the moment chat opens.
