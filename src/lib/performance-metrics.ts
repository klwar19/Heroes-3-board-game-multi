...
/**
 * Whether this session actually records metrics. Exported so hot-path callers
 * can skip building an expensive metric payload (e.g. byte-sizing a ~48 KiB
 * snapshot frame) when sampling is off — the default — instead of computing it
 * only for `recordPerformanceMetric` to discard.
 */
export const metricsSampled = sampleRate > 0 && Math.random() < sampleRate;
...
export function observeBrowserResponsiveness(): () => void {
  if (!metricsSampled || typeof PerformanceObserver === "undefined") return () => {};
